...
async def iniciar_scraping(
    ...
    """
    Recibe la URL, extrae el ASIN, revisa si ya existe en SQLite (Vía Rápida),
    o lanza el scraping profundo en segundo plano.
    """
    ...
    # === VÍA RÁPIDA: El producto ya está en SQLite ===
    producto_existente = obtener_producto(asin)
    if producto_existente:
        ...
        return {
            ...
            "asin": asin,
            "sesion_id": sesion_id,
            "mensaje": "Producto recuperado de la base de datos local."
        }

    # === VÍA LENTA: Scraping Nuevo ===
    # Validamos token de Apify (Tu validación original)
    ...
    # Lanzamos el trabajo en background
...
async def consultar_estado_scraping(
    ...
    Si ya terminó, genera la sesión automáticamente.
    """
    estado_actual = estados_tareas.get(asin, "no_encontrado")
    
    if estado_actual == "completado":
        # Creamos la sesión para entregarle el ID al frontend
